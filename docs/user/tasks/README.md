# OttoFlow Tasks

Step-by-step guides for common tasks with OttoFlow.

## Table of Contents

- [Installation](installation.md) - Install OttoFlow via Helm or manifests
- [Getting Started](getting-started.md) - Create your first workflow
- [MCP Servers and Tool Calls](mcp-servers-and-tools.md) - Configure MCP servers and call MCP tools from workflow steps
- [Triggers](triggers.md) - Setting up cron, event, and webhook triggers
- [Custom Metrics](custom-metrics.md) - Defining custom Prometheus metrics from workflows

For step types (resource queries, agents, mutate, forEach, and more), see the [Workflow API reference](../reference/api/workflow.md#step-types-one-per-step). For agent configuration, see the [Agent API reference](../reference/api/agent.md).

---

## Quick Links

- **[Concepts](../concepts/)** - Understanding OttoFlow architecture
- **[Reference](../reference/)** - API and CEL function reference
- **[Examples](../../../samples/workflows/)** - 80+ sample workflows
